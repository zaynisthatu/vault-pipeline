...
flowchart TD
    ...
    subgraph HOST ["Host — WSL Ubuntu"]
        G[hf-data/\n10k+ media files] --> H[npx tsx src/indexer.ts\n--folder /mnt/d/.../hf-data]
        H --> I[(vault.db\nSQLite · 1549 posts\nLinux absolute paths)]
    end

    subgraph CONTAINER ["Running Container — port 7860"]
        J[Express API + React SPA\nSTATELESS]
        K[/app/vault.db]
    ...
    I -.->|"-v vault.db:/app/vault.db"| K
    G -.->|"-v hf-data:hf-data:ro"| L
    CONTAINER --> M[✅ localhost:7860\n1549 posts · videos · thumbnails]
```
...
```mermaid
flowchart LR
    A[node:20-slim\nsets NODE_ENV=production] -->|causes| B[npm ci skips\ndevDependencies]
    B -->|result| C[vite missing\nfrom node_modules/.bin]
    C --> D[❌ build fails]

    E[Fix: ENV NODE_ENV=development\nin builder stage] -->|forces| F[npm ci installs\nall deps including vite]
    F --> G[✅ vite found]
```
...
```mermaid
flowchart TD
    A[esbuild --format=cjs] -->|outputs| B["module.exports = ..."]
    C["package.json\ntype: module"] -->|tells Node| D[treat .js as ESM]
    B & D --> E[❌ conflict at runtime]

    F["Fix: --out-extension:.js=.cjs"] -->|outputs| G[dist/server.cjs]
    G -->|Node treats .cjs as| H[✅ CommonJS always]
```
...
sequenceDiagram
    ...
    C-->>C: ❌ tries to load /src/main.tsx
```
...
```mermaid
flowchart TD
    A[DB indexed on Windows] -->|stores| B["D:\\New folder\\extraction\\hf-data\\...mp4"]
    C[Container = Linux] -->|has no| D["D:\\ path"]
    B -->|fs.existsSync fails| E[❌ 404 all media]

    F[Re-index from WSL\n--folder /mnt/d/.../hf-data] -->|stores| G["/mnt/d/New folder/extraction/hf-data/...mp4"]
    H["docker run -v\n/mnt/d/...:/mnt/d/...:ro"] -->|exact path match| I[✅ media streams]
```
...
<!-- TODO: Add screenshot of indexer output — 1,549 posts indexed -->
